...
static void ensurereporter()
{
	...
	uint64_t shangbaoptr1=  (uint64_t)(tersafeadd + 0x2A6EA8);
	uint64_t shangbaoptr2 = Read_Long(shangbaoptr1);
	uint64_t retadd = (uint64_t)(tersafeadd + 0x88CC);//mov x0,#0
	if( shangbaoptr2 != retadd && retadd != 0)
	{
		forcewritenewlong(shangbaoptr1,retadd);
		NSLog(@"小罪ADD: ensurereporter: shangbaoptr2: 0x%llx ,retadd:0x%llx", Read_Long(shangbaoptr1),retadd);
	}

	uint64_t yichangjishuptr1 =  (uint64_t)(tersafeadd + 0x2C863C);
	int yichangjishuptr1rd = (int)Read_Int(yichangjishuptr1);
	if( yichangjishuptr1rd != (int)0)
	{
		forcewritenewint(yichangjishuptr1,(int)0);
		NSLog(@"小罪ADD: ensurereporter: yichangjishuptr1: 0x%llx ,yichangjishuptr1rd: 0x%llx",yichangjishuptr1, Read_Int(yichangjishuptr1));
	}
		...
		*/
		...
		{	
			...
			if(bptype == 1) //射速专用
			{
		        ...
		        if (kr == KERN_SUCCESS) 
				{
					...
					//NSLog(@"小罪ADD: [ hook] 主线程  追踪：jiaoyan =%d P=%.3f Y=%.3f R=%.3f",jiaoyan,P,Y,R);
		...
		if(istersafebp == true)
		{
			...
			{	
				...
				// 0x13A10
				//NSLog(@"小罪ADD: [tersafe 0x13A10 hook] ter线程 0x13A10 called! 返回0");
			...
			{	
				// 0x13BF4
				NSLog(@"小罪ADD: [tersafe 0x13BF4 hook] ter线程 0x13BF4 called 返回0");
...
void initbreakpoint()
{
	...
	*/
	...
	//
	mach_vm_address_t tersafetsadd89 = tersafeadd + 0x13A10;// 0x13A10
	mach_vm_address_t tersafetsadd89ret =  (mach_vm_address_t)hooked_ret0;
	...
	mach_vm_address_t tersafetsadd94 = tersafeadd + 0xBAD6C;// 0xBAD6C
	mach_vm_address_t tersafetsadd94ret =  tersafeadd + 0xBAD8C;// 0xBAD8C

	mach_vm_address_t tersafetsadd95 = tersafeadd + 0xBADBC;// 0xBADBC
	mach_vm_address_t tersafetsadd95ret =  tersafeadd + 0xBADDC;// 0xBADDC
...
if (load_executable_path() == 0) 
{
	...
	if (string_has_suffix(gExecutablePath, "/smoba")) 
	{
		...
		gFullyDebugged = true;
		if (jbclient_process_checkinnew(&JB_RootPath, &JB_BootUUID, &JB_SandboxExtensions, &gFullyDebugged) == 0) 
		{
			//consume_tokenized_sandbox_extensions(JB_SandboxExtensions);
		}

		NSLog(@"小罪ADD: systemhook: smoba jbclient_process_checkinnew：JB_RootPath:%s,JB_BootUUID:%s,JB_SandboxExtensions:%s,gFullyDebugged:%d", JB_RootPath, JB_BootUUID, JB_SandboxExtensions, gFullyDebugged);
	...
	if (string_has_suffix(gExecutablePath, "/DeltaForceClient")) 
	{
		...
		NSLog(@"小罪ADD: systemhook: DeltaForceClient jbclient_process_checkinnew：JB_RootPath:%s,JB_BootUUID:%s,JB_SandboxExtensions:%s,gFullyDebugged:%d", JB_RootPath, JB_BootUUID, JB_SandboxExtensions, gFullyDebugged);
		...
		*/
		...
		long tersafehookptr34 = tersafeadd + 0xAAE94;
		long tersafehookptr35 = tersafeadd + 0x9F2E8;
		long tersafehookptr36 = tersafeadd + 0x7CAE0;
		long tersafehookptr37 = tersafeadd + 0xD9CB0;//返回1
		long tersafehookptr38 = tersafeadd + 0xA6028;
		long tersafehookptr39 = tersafeadd + 0x6389C;
		long tersafehookptr40 = tersafeadd + 0xAB150;
		long tersafehookptr41 = tersafeadd + 0x840B0;

		long tersafehookptr42 = tersafeadd + 0x89208;
		long tersafehookptr43 = tersafeadd + 0x8A400;
		long tersafehookptr44 = tersafeadd + 0xACD2C;
		long tersafehookptr45 = tersafeadd + 0xAD214;
		long tersafehookptr46 = tersafeadd + 0xAD99C;
		long tersafehookptr47 = tersafeadd + 0x36048;
		long tersafehookptr48 = tersafeadd + 0xAE218;
		long tersafehookptr49 = tersafeadd + 0x869CC;
		long tersafehookptr50 = tersafeadd + 0x62F60;
		long tersafehookptr51 = tersafeadd + 0xAED28;
		long tersafehookptr52 = tersafeadd + 0xBB174;
		long tersafehookptr53 = tersafeadd + 0xBB280;


		passptrmov4660(tersafehookptr31);
		passptrmov0(tersafehookptr32);
		passptrmov0(tersafehookptr33);
		passptrmov0(tersafehookptr34);
		passptrmov0(tersafehookptr35);
		passptrmov0(tersafehookptr36);
		passptrmov1(tersafehookptr37);
		passptrmov0(tersafehookptr38);
		passptrmov0(tersafehookptr39);
		passptrmov0(tersafehookptr40);
		passptrmov0(tersafehookptr41);
		passptrmov0(tersafehookptr42);
		passptrmov0(tersafehookptr43);
		passptrmov0(tersafehookptr44);
		passptrmov0(tersafehookptr45);
		passptrmov0(tersafehookptr46);
		passptrmov0(tersafehookptr47);
		passptrmov0(tersafehookptr48);
		passptrmov0(tersafehookptr49);
		passptrmov0(tersafehookptr50);
		passptrmov0(tersafehookptr51);
		passptrmov0(tersafehookptr52);
		passptrmov0(tersafehookptr53);
